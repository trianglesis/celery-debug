...
class DjangoFixup(object):
    ...
    def now(self, utc=False):
        octolog.error("<=DjangoFixup=> now -> datetime.utcnow() %s", datetime.utcnow())
        octolog.error("<=DjangoFixup=> now -> utc %s", utc)
        octolog.error("<=DjangoFixup=> now -> self._now() %s", self._now())
        # return datetime.now() if utc else self._now()
