...
class ScheduleEntry(object):
    ...
    def default_now(self):
        octolog.info("<=Celery Beat=> BEAT ============================== ONCE ON START")
        octolog.info("<=Celery Beat=> BEAT ============================== ONCE ON START")
        octolog.info("<=Celery Beat=> BEAT ============================== ONCE ON START")
        octolog.info("<=Celery Beat=> BEAT ============================== ONCE ON START")
        ...
        try:
            if self.schedule.now():
                octolog.error("<=Celery Beat=> default_now - > self.schedule.now() %s", self.schedule.now())
            if self.schedule:
                octolog.error("<=Celery Beat=> default_now - > self.schedule %s", self.schedule)
            if self.app.now():
                octolog.error("<=Celery Beat=> default_now - > self.app.now() %s", self.app.now())
            if self.last_run_at:
                octolog.error("<=Celery Beat=> default_now - > self.last_run_at %s", self.last_run_at)

        except Exception as e:
            log.warning("<=Celery Beat=> default_now CANNOT LOG THE TIME! %s", e)

        """
            [2018-06-22 08:21:38,155: ERROR/MainProcess] beat.py default_now - > self.schedule.now() 2018-06-22 07:21:38.155015+00:00
            [2018-06-22 08:21:38,155: ERROR/MainProcess] beat.py default_now - > self.schedule <crontab: 0 4
             * *
              * (m/h/d/dM/MY), UTC>
            [2018-06-22 08:21:38,156: ERROR/MainProcess] beat.py default_now - > self.app.now() 2018-06-22 07:21:38.155768+00:00
        """
...
class Scheduler(object):
    ...
    def apply_entry(self, entry, producer=None):
        octolog.info('Scheduler: Sending due task %s (%s)', entry.name, entry.task)
        try:
            result = self.apply_async(entry, producer=producer, advance=False)
        except Exception as exc:  # pylint: disable=broad-except
            octolog.error('Message Error: %s\n%s',
                  exc, traceback.format_stack(), exc_info=True)
        else:
            # Was debug
            octolog.info('%s sent. id->%s', entry.task, result.id)
    ...
    def _do_sync(self):
        try:
            # Was debug
            octolog.info('beat: Synchronizing schedule...')
            self.sync()
        finally:
    ...
    def _ensure_connected(self):
        ...
        def _error_handler(exc, interval):
            octolog.error('beat: Connection error: %s. '
...
class PersistentScheduler(Scheduler):
    ...
    def _destroy_open_corrupted_schedule(self, exc):
        octolog.error('Removing corrupted schedule file %r: %r',
    ...
    def setup_schedule(self):
        ...
        if stored_tz is not None and stored_tz != tz:
            octolog.warning('Reset: Timezone changed from %r to %r', stored_tz, tz)
            self._store.clear()   # Timezone changed, reset db!
        utc = self.app.conf.enable_utc
        stored_utc = self._store.get(str('utc_enabled'))
        if stored_utc is not None and stored_utc != utc:
            choices = {True: 'enabled', False: 'disabled'}
            octolog.warning('Reset: UTC changed from %s to %s',
        ...
        for _ in (1, 2):
            try:
                ...
                if str('__version__') not in self._store:
                    octolog.warning('DB Reset: Account for new __version__ field')
                    self._store.clear()   # remove schedule at 2.2.2 upgrade.
                elif str('tz') not in self._store:
                    octolog.warning('DB Reset: Account for new tz field')
                    self._store.clear()   # remove schedule at 3.0.8 upgrade
                elif str('utc_enabled') not in self._store:
                    octolog.warning('DB Reset: Account for new utc_enabled field')
...
class Service(object):
    ...
    def start(self, embedded_process=False):
        octolog.info('beat: Starting...')
        # Was debug
        _max_interval = humanize_seconds(self.scheduler.max_interval)
        octolog.info('beat: Ticking with max interval->%s', str(_max_interval))
        ...
        try:
            while not self._is_shutdown.is_set():
                ...
                if interval and interval > 0.0:
                    # Was debug
                    woke_up = humanize_seconds(interval, prefix='in ')
                    octolog.info('beat: Waking up %s.', str(woke_up))
                    time.sleep(interval)
    ...
    def stop(self, wait=False):
        octolog.info('beat: Shutting down...')
        self._is_shutdown.set()
