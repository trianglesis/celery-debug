...
from celery.utils.time import (get_exponential_backoff_interval, timezone, to_utc)  # Base
...
# octolog.error("<=Celery App Base=> celery.utils.time %s", timezone)
# <=Celery App Base=> celery.utils.time <celery.utils.time._Zone object at 0x7fdce2dea780>

"""
<=APP BASE=>    timezone.get_timezone <bound method _Zone.get_timezone of <celery.utils.time._Zone object at 0x7f96d70adf28>>
<=APP BASE=>    timezone.to_local_fallback <bound method _Zone.to_local_fallback of <celery.utils.time._Zone object at 0x7f96d70adf28>>
<=APP BASE=>    timezone.to_system <bound method _Zone.to_system of <celery.utils.time._Zone object at 0x7f96d70adf28>>
<=APP BASE=>    timezone.tz_or_local <bound method _Zone.tz_or_local of <celery.utils.time._Zone object at 0x7f96d70adf28>>
"""
# try:
#     octolog.error("<=APP BASE=> \t timezone.get_timezone %s", timezone.get_timezone)
#     # octolog.error("<=APP BASE=> \t timezone.local %s", timezone.local)
#     # octolog.error("<=APP BASE=> \t timezone.to_local_fallback %s", timezone.to_local_fallback)
#     # octolog.error("<=APP BASE=> \t timezone.to_system %s", timezone.to_system)
#     # octolog.error("<=APP BASE=> \t timezone.tz_or_local %s", timezone.tz_or_local)
#     # octolog.error("<=APP BASE=> \t timezone.utc %s", timezone.utc)
...
def _after_fork_cleanup_app(app):
    ...
    try:
        ...
        octolog.info('after forker raised exception: %r', exc, exc_info=1)
...
class Celery(object):
    ...
    def now(self):
        ...
        """
        octolog.info("<=Celery App Base=> NOW(): <--------------------------------> CYCLE NOW <-------------------------------->")
        now_in_utc = to_utc(datetime.now())
        # now_in_utc = to_utc(datetime.utcnow())
        octolog.error("<=Celery App Base=> NOW(): now_in_utc %s", now_in_utc)
        octolog.error("<=Celery App Base=> NOW(): Django %s", dj_timezone.now)
        octolog.error("<=Celery App Base=> NOW(): UTC Replace %s", datetime.utcnow().replace(tzinfo=utc))
        try:
            now_utc_obj = now_in_utc.astimezone(self.timezone)
            octolog.error("<=Celery App Base=> NOW(): now_utc_obj %s", now_utc_obj)
            return now_utc_obj
        except TypeError:
            now_utc_val = now_in_utc.astimezone(self.timezone())
            octolog.error("<=Celery App Base=> NOW(): now_utc_val %s", now_utc_val)
            return now_utc_val
    ...
    def uses_utc_timezone(self):
        ...
        check_tz_use = self.timezone == timezone.utc
        octolog.error("<=Celery App Base=> USES_UTC_TIMEZONE(): self.timezone '%s'", self.timezone)
        octolog.error("<=Celery App Base=> USES_UTC_TIMEZONE(): timezone.utc  '%s'", timezone.utc)
        octolog.error("<=Celery App Base=> USES_UTC_TIMEZONE(): check_tz_use   %s", check_tz_use)
        return check_tz_use
    ...
    def timezone(self):
        ...
        [2018-06-23 18:25:54,620: INFO/MainProcess] <=Celery App Base=> TIMEZONE(): conf.timezone: Europe/London
        [2018-06-23 18:25:54,620: INFO/MainProcess] <=Celery App Base=> TIMEZONE(): set_get_tz: Europe/London
        ...
        if not conf.timezone:
            octolog.info("<=Celery App Base=> TIMEZONE(): if not conf.timezone:")
            if conf.enable_utc:
                octolog.info("<=Celery App Base=> TIMEZONE(): timezone.utc %s", timezone.utc)
                return timezone.utc
            else:
                octolog.info("<=Celery App Base=> TIMEZONE(): timezone.local %s", timezone.local)
                return timezone.local
        set_get_tz = timezone.get_timezone(conf.timezone)
        octolog.info("<=Celery App Base=> TIMEZONE(): conf.timezone: %s", conf.timezone)
        octolog.info("<=Celery App Base=> TIMEZONE(): set_get_tz: %s", set_get_tz)
        # return timezone.utc
        return set_get_tz
