...
class BaseSchedule(object):
    ...
    def now(self):
        # OK!
        # <=Celery Schedules=> now -> self.app.now() 2018-06-22 11:48:04.949531+01:00
        # <=Celery Schedules=> now -> self.nowfun() 2018-06-22 11:48:04.952139+01:00
        nowfun = self.nowfun()
        app_now = self.app.now()
        octolog.error("<=Celery Schedules=> now -> self.nowfun() %s", str(nowfun))
        octolog.error("<=Celery Schedules=> now -> self.app.now() %s", str(app_now))
    ...
    @cached_property
    def tz(self):
        octolog.info("<=schedules.py=>self.app.timezone %s", self.app.timezone)
    ...
    def to_local(self, dt):
        octolog.error("<=Celery Schedules=> to_local -> dt %s", dt)
        if not self.utc_enabled:
            octolog.error("<=Celery Schedules=> to_local -> timezone.to_local_fallback(dt) %s", timezone.to_local_fallback(dt))
...
class solar(BaseSchedule):
    ...
    def remaining_estimate(self, last_run_at):
        ...
        """
        last_run_at = self.maybe_make_aware(last_run_at)
        octolog.debug("<=Schedules.py=> REMAINING_ESTIMATE(): last_run_at: %s", last_run_at)
        last_run_at_utc = localize(last_run_at, timezone.utc)
        octolog.debug("<=Schedules.py=> REMAINING_ESTIMATE(): last_run_at_utc: %s", last_run_at_utc)
